...
// Ao carregar a página, atribui eventos aos botões por meio dos seus ids
...
function inserirNumero(event) {
    ...
    if (isNaN(inputResultado.value)) {
        ...
        // senão, adicioa o valor aos demais
    } else {
        // Se o valor não for zero, substitui pelo valor do visor pelo número clicado
        if(inputResultado.value == 0) {
            inputResultado.value = event.target.textContent;
            // senão, adicioa o número aos digitos no visor
        } else {
            inputResultado.value += event.target.textContent;
        }
    }
...
function dividir(valor1, valor2) {
    if(valor2 === 0) {
        return "Erro, não é possivel dividir um número por zero!";
    } else {
        return valor1 / valor2;
    }
}
